...
from Dataset import TripartiteData, QuadripartiteData, TripartiteCatData, QuadripartiteCatData, FivepartiteData
from QDA import TripartiteQDA, QuadripartiteQDA, FivepartiteQDA
from sharedCNN import TripartiteModel, QuadripartiteModel, FivepartiteModel
...
'''-----------------Five-partite cases-----------------'''
# xtrain, ytrain, xtest, ytest = FivepartiteData(num_states=7000)
# print("The number of samples before QDA:" + str(xtrain.shape[0]))

# xtrain, ytrain = FivepartiteQDA(xtrain, ytrain, n=10) # Quantum Data Augmentation (QDA) for five-partite states
# print("The number of samples after QDA:" + str(xtrain.shape[0]))

# model = FivepartiteModel()
...
with tf.device('/gpu:0'):
    history = model.fit( 
        ...
        # # for five-partite states
        # [xtrain[:, 0:4, :, :].transpose(0, 2, 3, 1), xtrain[:, 4:8, :, :].transpose(0, 2, 3, 1), xtrain[:, 8:12, :, :].transpose(0, 2, 3, 1), xtrain[:, 12:16, :, :].transpose(0, 2, 3, 1),
        # xtrain[:, 16:20, :, :].transpose(0, 2, 3, 1)],ytrain,    
...
# # for five-partite states
# ypred = model.predict([xtrain[:, 0:4, :, :].transpose(0, 2, 3, 1), xtrain[:, 4:8, :, :].transpose(0, 2, 3, 1), xtrain[:, 8:12, :, :].transpose(0, 2, 3, 1), xtrain[:, 12:16, :, :].transpose(0, 2, 3, 1), xtrain[:, 16:20, :, :].transpose(0, 2, 3, 1)])
